...
RetrieverCallable = Callable[[], SideInputResponse]
_PROCESS_COUNT = multiprocessing.cpu_count()
MAX_THREADS = int(os.getenv("MAX_THREADS", 0)) or (_PROCESS_COUNT * 4)


class SideInput(sideinput_pb2_grpc.UserDefinedSideInputServicer):
    """
    Provides an interface to write a User Defined Side Input (UDSideInput)
    which will be exposed over gRPC.

    Args:
        retrieve_side_input_handler: Function callable following the type signature of RetrieverCallable
    ...
    >>> from pynumaflow.sideinput import SideInputResponse
    >>> def my_handler() -> SideInputResponse:
    ...   response = SideInputResponse()
    ...   return response
    ...
    def __init__(
        self,
        retrieve_side_input_handler: RetrieverCallable,
        sock_path=SIDE_INPUT_SOCK_PATH,
        max_message_size=MAX_MESSAGE_SIZE,
        max_threads=MAX_THREADS,
    ):
        if retrieve_side_input_handler is None:
            raise ValueError("retrieve_side_input_handler cannot be None")
        self.__retrieve_handler: RetrieverCallable = retrieve_side_input_handler
    ...
    def RetrieveSideInput(
        ...
        try:
            ...
            return sideinput_pb2.SideInputResponse(value=None)

        return sideinput_pb2.SideInputResponse(value=rspn.value)
    ...
    def start(self) -> None:
        ...
        sideinput_pb2_grpc.add_UserDefinedSideInputServicer_to_server(
