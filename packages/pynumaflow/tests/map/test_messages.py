...
def test_map_class_call():
    ...
    """
    mapper_instance = ExampleMapper()
    # make a call to the class directly
    ret = mapper_instance([], None)
    assert mock_message() == ret[0].value
    # make a call to the handler
    ret_handler = mapper_instance.handler(keys=[], datum=None)
    #
