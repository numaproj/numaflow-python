use crate::accumulate::{Datum as PyDatum, Message as PyMessage, PyAsyncDatumStream};
...
pub(crate) struct PyAccumulatorCreator {
    /// handle to Python event loop
    pub(crate) event_loop: Arc<Py<PyAny>>,
    /// Python class to instantiate per key
    pub(crate) py_creator: Arc<Py<PyAny>>,
    /// optional tuple of positional args
    pub(crate) init_args: Option<Arc<Py<PyAny>>>,
}

pub(crate) struct PyAccumulatorRunner {
    /// handle to Python event loop
    pub(crate) event_loop: Arc<Py<PyAny>>,
    /// Instance of class per key
    pub(crate) py_instance: Arc<Py<PyAny>>,
}

impl accumulator::AccumulatorCreator for PyAccumulatorCreator {
    type A = PyAccumulatorRunner;
    ...
    fn create(&self) -> Self::A {
        ...
        .expect("failed to instantiate Python accumulator class");
...
#[tonic::async_trait]
impl accumulator::Accumulator for PyAccumulatorRunner {
    async fn accumulate(
        &self,
        mut input: tokio::sync::mpsc::Receiver<accumulator::AccumulatorRequest>,
        output: tokio::sync::mpsc::Sender<accumulator::Message>,
    ) {
        ...
        // obj.handler(datums: AsyncIterable[Datum]) -> AsyncIterator[Message]
        let agen_obj = Python::attach(|py| {
            ...
            // Call handler method
            let agen = obj
                .call_method1(py, "handler", (stream,))
        ...
        while let Some(item) = stream.next().await {
            match item {
                Ok(py_msg) => {
                    let out: accumulator::Message = py_msg.into();
                ...
                Err(e) => {
                    ...
                    eprintln!("Python async iteration error in accumulator: {:?}", e);
                    break;
                }
            }
        }
...
/// Start the accumulator server by spinning up a dedicated Python asyncio loop and wiring shutdown.
pub(super) async fn start(
    ...
    let creator = PyAccumulatorCreator {
    ...
    println!("Numaflow Core (accumulator) has shutdown...");
