...
class UserDefinedSideInputStub(object):
    """UserDefinedSideInput is the gRPC service for user-defined Side Inputs.
...
def add_UserDefinedSideInputServicer_to_server(servicer, server):
    rpc_method_handlers = {
    ...
    generic_handler = grpc.method_handlers_generic_handler(
        "sideinput.v1.UserDefinedSideInput", rpc_method_handlers
    )
    server.add_generic_rpc_handlers((generic_handler,))


# This class is part of an EXPERIMENTAL API.
class UserDefinedSideInput(object):
    """UserDefinedSideInput is the gRPC service for user-defined Side Inputs.
    It is used to propagate changes in the values of the provided Side Inputs
    which allows access to slow updated data or configuration without needing to retrieve
    it during each message processing.
